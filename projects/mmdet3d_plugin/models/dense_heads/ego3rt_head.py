...
class Ego3RTCenterHead(BaseModule):
    """CenterHead for Ego3RT
    ...
    def __init__(self,
        ...
        super(Ego3RTCenterHead, self).__init__(init_cfg=init_cfg)
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if self.test_cfg['nms_type'] == 'circle':
                ...
                for i in range(batch_size):
                    ...
                    if len(self.test_cfg['min_radius'][task_id]) == 1:
                        keep = torch.tensor(
                            circle_nms(
                                boxes.detach().cpu().numpy(),
                                self.test_cfg['min_radius'][task_id][0],
                        ...
                        scores_task = []
                        labels_task = []
                        for obj_id, obj_radius in enumerate(self.test_cfg['min_radius'][task_id]):
                            boxes3d_obj = boxes3d[labels==obj_id]
                            scores_obj = scores[labels==obj_id]
                            boxes_obj = boxes[labels==obj_id]
                            labels_obj = labels[labels==obj_id]
                            keep_obj = torch.tensor(circle_nms(
                                boxes_obj.detach().cpu().numpy(),
                                obj_radius,
                                post_max_size=self.test_cfg['post_max_size']),
                                dtype=torch.long,
                                device=boxes.device)
                            boxes3d_task.append(boxes3d_obj[keep_obj])
                            scores_task.append(scores_obj[keep_obj])
                            labels_task.append(labels_obj[keep_obj])
                        boxes3d = torch.cat(boxes3d_task, dim=0)
                        scores = torch.cat(scores_task, dim=0)
                        labels = torch.cat(labels_task, dim=0)
